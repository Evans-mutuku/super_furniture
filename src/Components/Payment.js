  ...
  return (
    <main className="bg-white flex justify-around p-5 mb-3 w-full">
      ...
      <div className="ml-20">
        ...
        {!hideButton && (
          <button
            className="bg-pink-500 mt-12 p-2 text-white rounded-sm"
  ...
  return (
    <main className="pt-24 mx-wd mx-auto">
      <h2 className="flex text-2xl">
        Items to pay{" "}
        <p onClick={() => navigate("/checkout")} className="text-pink-500 ml-3">
          ( {basket?.length} items)
        </p>{" "}
      </h2>
        ...
        <div className="w-1/3">
          <form className="ml-32 w-full" onSubmit={handleSubmit}>
            ...
            <div className="mt-3 ">
              ...
              />
              <button
                className="bg-pink-500 cursor-pointer p-2 mt-3 w-full text-white"
                disabled={processing || disabled || succeeded}
              >
                <span>{processing ? <p>Processing</p> : "Purchase Now"} </span>
              </button>
            </div>
            {error && <div className="text-pink-700">{error}</div>}
          </form>
        </div>
      </section>
      <section></section>
